...
# ── TimelineItem keyframes: a documented Resolve API gap ─────────────────────
#
# Resolve's scripting API has no TimelineItem keyframe methods, on any edition
# or build: AddKeyframe, ModifyKeyframe, DeleteKeyframe, GetKeyframeCount,
# GetKeyframeAtIndex, GetPropertyAtKeyframeIndex and SetKeyframeInterpolation
# do not exist — the bundled API reference's only keyframe surface is
# Resolve.Get/SetKeyframeMode, which switches the Edit-page UI mode. The tools
# below used to call those phantom methods and could never succeed anywhere;
# over the bridge the failure even surfaced as a misleading per-build
# capability error. They now say so, and point at the route that works.
# See docs/reference/api-limitations.md (TimelineItem keyframes).

_KEYFRAME_API_GAP = (
    "Error: Resolve's scripting API has no TimelineItem keyframe methods, on "
    "any edition or build, so this tool can never succeed. To animate "
    "transform/opacity/crop from a script, keyframe the clip's Fusion comp "
    "instead (fusion_comp add_keyframe on e.g. a Transform tool). Static "
    "values remain settable via set_timeline_item_transform/_crop/_composite. "
    "See docs/reference/api-limitations.md (TimelineItem keyframes)."
)
...
def get_timeline_item_keyframes(timeline_item_id: str, property_name: str) -> Dict[str, Any]:
    """Unsupported: Resolve's API cannot read timeline-item keyframes; explains the gap."""
    return {"error": _KEYFRAME_API_GAP, "item_id": timeline_item_id}


@mcp.tool()
def add_keyframe(timeline_item_id: str, property_name: str, frame: int, value: float) -> str:
    """Unsupported: Resolve's API cannot add Edit-page keyframes; use the Fusion route.

    Returns an explanation of the API gap and the working alternative
    (fusion_comp add_keyframe on the clip's Fusion comp).
    """
    return _KEYFRAME_API_GAP


@mcp.tool()
def modify_keyframe(timeline_item_id: str, property_name: str, frame: int, new_value: float = None, new_frame: int = None) -> str:
    """Unsupported: Resolve's API cannot modify Edit-page keyframes; use the Fusion route."""
    return _KEYFRAME_API_GAP


@mcp.tool()
def delete_keyframe(timeline_item_id: str, property_name: str, frame: int) -> str:
    """Unsupported: Resolve's API cannot delete Edit-page keyframes; use the Fusion route."""
    return _KEYFRAME_API_GAP


@mcp.tool()
def set_keyframe_interpolation(timeline_item_id: str, property_name: str, frame: int, interpolation_type: str) -> str:
    """Unsupported: Resolve's API cannot set keyframe interpolation; use the Fusion route."""
    return _KEYFRAME_API_GAP
